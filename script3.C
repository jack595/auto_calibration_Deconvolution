...
void script3(int pmax = 3, int pnum = 2000, double pwidth = 0.35, double pmu = 0.05, int ploop = 0)
{
  ...
  TF1* tf2 = new TF1("tf2", "gaus(0)", 0.3, 1.4);
  TF1* tf;
  ...
  for (int i = 0; i < loopN; i++) {
    ...
    if (tf->GetChisquare() / tf->GetNDF() > 4 ) {
...
void setPMTMap()
{
  ...
  TString filename =
      "/junofs/users/zhangxt/20inch/rec/deconvolution/runs/20191009sample/"
      "PMTPos_Acrylic_with_chimney.csv";
  ifstream sfile;
