...
#include "/afs/ihep.ac.cn/users/l/luoxj/workfs_juno_5G/include/myincludes.h"
double timeinteval = 400.e-9;
void calib_gain()
{
  int npmt = 17612;
...
//
  ...
  for (int i = 0; i < npmt; i++) outf << i << "\t" << 0 << "\t" << gainth[nn - 1]->GetBinContent(i + 1) << "\t" << 0 << "\t" << 0 << "\t" << 0 << "\t" << endl;
}
