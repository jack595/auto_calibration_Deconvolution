...
void calib_dn()
{
  ...
  // TString fname[nn] = {"AmC", "Co60", "Cs137", "Ge68", "photon_22000", "photon_2200", "Averaged"};
  // TString fname2[nn] = {"AmC", "^{60}Co", "^{137}Cs", "^{68}Ge", "#mu = 0.1", "#mu = 0.01", "Averaged"};
  //
  //TString fname[nn] = {"C14", "photon_22000", "photon_11522", "Averaged"};
  //TString fname2[nn] = {"^{14}C", "#mu = 0.1", "#mu = 0.01", "Averaged"};
  ...
  for (int i = 0; i < nn - 1; i++) {
    f[i] = new TFile(fname[i] + "_outf.root", "read");
    darkrateth[i] = (TH1F*)f[i]->Get("darkCharge");
  }
  for (int i = 0; i < nn - 1; i++) {

    fc[i] = new TFile(fname[i] + "_calib0/step4/user_calibCorr.root", "read");
